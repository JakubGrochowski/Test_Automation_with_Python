...
class TestFuzzyMath:
    ...
    def test_non_int_input_for_num2(self):
        with pytest.raises(Exception) as exec_info:
            fuzzy_math(2, '+', 'hi')
        assert 'fuzzy math on ints' in str(exec_info)
    ...
    def test_multiplication_with_negative_result(self):
        assert 'negative number' in fuzzy_math(-30, '*', 2)

    def test_multiplication_with_small_result(self):
        assert 'small number' in fuzzy_math(3, '*', 3)
    ...
    def test_multiplication_with_large_result(self):
        assert 'really big' in fuzzy_math(10, '*', 10)

    def test_invalid_operator(self):
        with pytest.raises(Exception) as exec_info:
            fuzzy_math(6, '&', 6)
            assert 'how to do math' in str(exec_info)
